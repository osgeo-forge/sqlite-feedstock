...
:: Add other DLL opts set in OSGeo4W build
set "opts=-DSQLITE_ENABLE_FTS3_PARENTHESIS=1 -DSQLITE_ENABLE_STAT3=1 -DSQLITE_ENABLE_UPDATE_DELETE_LIMIT=1"

:: Note: running 'clean' target first, for when debugging build
...
nmake /f Makefile.msc DYNAMIC_SHELL=1 "OPTS=%opts%" core
if %errorlevel% neq 0 echo Failed to make core & exit /b 1

nmake /f Makefile.msc install
if %errorlevel% neq 0 echo Failed to make install & exit /b 1
